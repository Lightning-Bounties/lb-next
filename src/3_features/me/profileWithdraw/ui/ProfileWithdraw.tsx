...
const ProfileWithdraw: React.FC<ProfileWithdrawProps> = ({ refetch }) => {
    ...
    const onFinishHandler = async (data: { payment_request: string }) => {
        try {
            ...
            api.success({
                message: 'Payment initiated successlly. But not confirmed yet',
            });
            await refetch();
            form.resetFields();
            ...
            if (invoice.startsWith('lnbc') || invoice.includes('lightning:')) {
                try {
                    ...
                    if (!parsed || !parsed.data) {
                        setError(
                            'Only standard BOLT11 invoices are supported in this form.',
                        );
                        return;
                    }
                    ...
                    const memoTag = boltData.tags.find(
                        (tag: { tagName: string; data: unknown }) =>
                            tag.tagName === 'description',
                    );
                    const hashTag = boltData.tags.find(
                        (tag: { tagName: string; data: unknown }) =>
                            tag.tagName === 'payment_hash',
                    );

                    const memo =
                        typeof memoTag?.data === 'string' ? memoTag.data : '';
                    const hash =
                        typeof hashTag?.data === 'string' ? hashTag.data : '';

                    const expiry =
                        (boltData.timeExpireDate ?? boltData.timestamp + 3600) *
                        1000;
                    ...
                    const errorMessage =
                        (err as Error).message || 'Failed to decode invoice.';
                    setError(
                ...
                setDecodedData(null);
                setError(
                    'Invoice type not recognized: expecting it to start with `lnbc` or `lightning:`.',
                );
            }
        } else {
            setIsButtonEnabled(false);
                ...
                <div style={{ marginTop: 16 }}>
                    ...
                    <Flex vertical gap="small">
                        ...
                        <Flex>
                            ...
                            <Typography.Text>
                                {decodedData.expiry.toLocaleString()}
                            </Typography.Text>
                        </Flex>
                        {decodedData.memo && (
                            <Flex>
