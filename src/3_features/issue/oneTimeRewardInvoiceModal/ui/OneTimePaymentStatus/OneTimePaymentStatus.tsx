...
const OneTimePaymentStatus: FC<OneTimePaymentStatusProps> = ({ issueId, checkingId }) => {
    ...
    const paymentCheck = async (issueId: string, checkingId: string) => {
        ...
        const response = await issueApi.checkOneTimePayment(issueId, checkingId)
        if (response.is_paid) {
            setIsPaid(true)
            api.success({
                message: 'Payment received; reward added! Refreshing in 5 secs...',
            })
            setTimeout(() => {  
    ...
    useEffect(() => {
        setIsPaid(false)
        queryClient.invalidateQueries({ queryKey: ['oneTimePaymentStatusChecker', issueId, checkingId] })
    }, [checkingId, issueId])
