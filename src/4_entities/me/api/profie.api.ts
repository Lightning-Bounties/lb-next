...
class ProfileApi {
    ...
    async getProfilePaymentsHistory(limit: number = 100) {
        const resp = await appApi.wallets.getWalletHistoryApiWalletHistoryGet({limit: limit})
        return resp
    }
    ...
    async withdrawFunds(opts: { paymentLink: string }) {
        const { success } = await appApi.wallets.withdrawSatsApiWalletWithdrawPost({
            requestBody: {
    ...
    async expireReward(rewardId: string) {
        const resp = await appApi.rewards.expireRewardApiRewardsExpirePost({
            requestBody: { id: rewardId }
        })
        return resp
    }
